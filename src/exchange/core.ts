...
export function handleTransfer(event: Transfer): void {
  ...
  // case where direct send first on AVAX withdrawals
...
export function handleSync(event: Sync): void {
  ...
  pancake.totalLiquidityAVAX = pancake.totalLiquidityAVAX.minus(pair.trackedReserveAVAX as BigDecimal);
  ...
  let t0DerivedAVAX = findAVAXPerToken(token0 as Token);
  token0.derivedAVAX = t0DerivedAVAX;
  token0.derivedUSD = t0DerivedAVAX.times(bundle.AVAXPrice);
  token0.save();

  let t1DerivedAVAX = findAVAXPerToken(token1 as Token);
  token1.derivedAVAX = t1DerivedAVAX;
  token1.derivedUSD = t1DerivedAVAX.times(bundle.AVAXPrice);
  token1.save();
  ...
  pancake.totalLiquidityAVAX = pancake.totalLiquidityAVAX.plus(trackedLiquidityAVAX);
  pancake.totalLiquidityUSD = pancake.totalLiquidityAVAX.times(bundle.AVAXPrice);
...
export function handleSwap(event: Swap): void {
  ...
  pancake.totalVolumeAVAX = pancake.totalVolumeAVAX.plus(trackedAmountAVAX);
  ...
  pancakeDayData.dailyVolumeAVAX = pancakeDayData.dailyVolumeAVAX.plus(trackedAmountAVAX);
  ...
  token0DayData.dailyVolumeAVAX = token0DayData.dailyVolumeAVAX.plus(amount0Total.times(token0.derivedAVAX as BigDecimal));
